...
struct PullUpdatesProbe {
    ...
    logical_txns: Vec<ProbeLogicalTxnData>,
}
...
#[derive(Clone, PartialEq, prost::Message)]
struct ProbeLogicalOp {
    #[prost(enumeration = "ProbeLogicalOpType", tag = "1")]
    op_type: i32,
    #[prost(string, tag = "2")]
    table_name: String,
    #[prost(sint64, tag = "3")]
    ...
    #[prost(optional, enumeration = "ProbeLogicalSchemaAction", tag = "8")]
    schema_action: Option<i32>,
    #[prost(optional, enumeration = "ProbeLogicalSchemaKind", tag = "9")]
    schema_kind: Option<i32>,
    #[prost(string, tag = "10")]
    schema_name: String,
}

#[derive(Clone, PartialEq, prost::Message)]
struct ProbeLogicalTxnData {
    #[prost(uint64, tag = "1")]
    end_offset: u64,
    #[prost(uint64, tag = "2")]
    commit_ts: u64,
    #[prost(message, repeated, tag = "3")]
    ops: Vec<ProbeLogicalOp>,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, prost::Enumeration)]
#[repr(i32)]
enum ProbeLogicalOpType {
    UpsertRow = 0,
    DeleteRow = 1,
    Schema = 2,
    UpdateHeader = 3,
...
async fn run_seeded(config: &Config, args: &RunArgs) -> Result<()> {
    ...
    let initial_generation = query_remote_generation(config).await?;
    let primary =
    ...
    assert_seeded_push_cdc_high_water(config, &primary, &state, 0, "initial seeded push").await?;
    let _bootstrap_generation = if env_flag_enabled("SYNC_EXAMPLE_FORCE_BOOTSTRAP_ROLLOVER") {
    ...
    if env_flag_enabled("SYNC_EXAMPLE_PROBE_BOOTSTRAP") {
    ...
    run_targeted_mvcc_logical_coverage_suite(config, dir.path(), &mut replicas, &mut state).await?;
...
async fn assert_scripted_current_generation_logical_pull_after_remote_drop(
    ...
    if probe.stream_kind != Some(PullUpdatesStreamKind::Logical) {
        bail!(
...
async fn run_replace_base_local_preservation_check(
    ...
    let mut replica = build_local_replica(replica_name.clone(), replica_path, config, true)
        .await
...
async fn assert_local_cdc_advanced(
    ...
    if let Some(previous) = state.local_cdc_high_water[replica_idx] {
        if current <= previous {
            bail!(
                "local pushable CDC high-water did not advance for {label}: previous={previous} current={current}"
            );
        }
    }
...
async fn assert_seeded_push_cdc_high_water(
    config: &Config,
    replica: &LocalReplica,
    state: &SeededScenarioState,
    replica_idx: usize,
    label: &str,
) -> Result<()> {
    let Some(expected) = state.local_cdc_high_water[replica_idx] else {
        return Ok(());
    };
...
async fn local_replayable_cdc_count(replica: &LocalReplica) -> Result<i64> {
    ...
    let floor = local_sync_change_id_for_replica(replica).await?.unwrap_or(-1);
...
async fn probe_pull_updates_stream(
    ...
    while offset < body.len() {
        ...
        if stream_kind == Some(PullUpdatesStreamKind::Logical) {
            logical_txns.push(
                ProbeLogicalTxnData::decode_length_delimited(&body[message_start..offset])
                    .with_context(|| {
                        format!(
                            "failed to decode logical pull-updates message {}",
                            trailing_messages + 1
                        )
                    })?,
            );
...
async fn probe_pull_updates_header(
    ...
    let stream_kind = ProbePullUpdatesStreamKind::try_from(header.stream_kind)
        ...
        .map(|kind| match kind {
            ...
            ProbePullUpdatesStreamKind::Logical => PullUpdatesStreamKind::Logical,
        });
    let apply_mode = ProbePullUpdatesApplyMode::try_from(header.apply_mode)
        .ok()
        .map(|mode| match mode {
...
async fn print_pull_updates_probe(config: &Config, client_revision: &str, label: &str) {
    match probe_pull_updates_stream(config, client_revision).await {
        Ok(probe) => eprintln!(
            "pull-updates probe for {label}: client_revision={} server_revision={} db_size={} stream_kind={:?} apply_mode={:?} trailing_messages={} logical_ops={}",
            client_revision,
            probe.server_revision,
            probe.db_size,
            probe.stream_kind,
            probe.apply_mode,
            probe.trailing_messages,
            count_logical_ops(&probe.logical_txns),
        ),
        Err(err) => eprintln!(
...
fn count_logical_ops(txns: &[ProbeLogicalTxnData]) -> usize {
    txns.iter().map(|txn| txn.ops.len()).sum()
}

fn assert_probe_has_row_upsert(probe: &PullUpdatesProbe, table: &str, rowid: i64) -> Result<()> {
    if probe.logical_txns.iter().any(|txn| {
        txn.ops.iter().any(|op| {
            op.op_type == ProbeLogicalOpType::UpsertRow as i32
                && op.table_name == table
                && op.rowid == rowid
        })
    }) {
    ...
    bail!(
        "logical probe missing upsert for table={} rowid={}; server_revision={}",
        table,
        rowid,
        probe.server_revision,
    )
...
fn assert_probe_has_schema_op(
    ...
    action: ProbeLogicalSchemaAction,
    kind: ProbeLogicalSchemaKind,
    name: &str,
) -> Result<()> {
    if probe.logical_txns.iter().any(|txn| {
        txn.ops.iter().any(|op| {
            op.op_type == ProbeLogicalOpType::Schema as i32
                && op.schema_action == Some(action as i32)
                && op.schema_kind == Some(kind as i32)
                && op.schema_name == name
        })
    }) {
        return Ok(());
    ...
    bail!(
        "logical probe missing schema op action={:?} kind={:?} name={}; server_revision={} logical_ops={}",
        action,
        kind,
        name,
        probe.server_revision,
        summarize_logical_ops(probe),
    )
}
...
fn assert_probe_has_table_schema_op(probe: &PullUpdatesProbe, name: &str) -> Result<()> {
    if probe.logical_txns.iter().any(|txn| {
        txn.ops.iter().any(|op| {
            op.op_type == ProbeLogicalOpType::Schema as i32
                && op.schema_kind == Some(ProbeLogicalSchemaKind::Table as i32)
                && op.schema_name == name
        })
    }) {
        return Ok(());
    ...
    bail!(
        "logical probe missing table schema op name={}; server_revision={} logical_ops={}",
        name,
        probe.server_revision,
        summarize_logical_ops(probe),
    )
...
fn summarize_logical_ops(probe: &PullUpdatesProbe) -> String {
    let mut parts = Vec::new();
    for txn in &probe.logical_txns {
        for op in &txn.ops {
            let op_type =
                ProbeLogicalOpType::try_from(op.op_type).map_or(
                    "unknown",
                    |op_type| match op_type {
                        ProbeLogicalOpType::UpsertRow => "upsert",
                        ProbeLogicalOpType::DeleteRow => "delete",
                        ProbeLogicalOpType::Schema => "schema",
                        ProbeLogicalOpType::UpdateHeader => "header",
                    },
                );
            parts.push(format!(
                "{} table={} rowid={} schema={:?}/{:?}/{}",
                op_type, op.table_name, op.rowid, op.schema_action, op.schema_kind, op.schema_name,
            ));
        }
    }
    parts.join("; ")
...
fn summarize_snapshot_diff(expected: &TableSnapshot, actual: &TableSnapshot) -> String {
    ...
    let only_actual = actual_ids
        ...
        .copied()
        .collect::<Vec<_>>();
    format!("only_remote={only_expected:?} only_local={only_actual:?}")
}
