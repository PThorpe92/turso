...
=======
    ...
    fn join_fuzz_inner(
        ...
        let (mut rng, seed) = rng_from_time_or_env();
        println!(
            "join_fuzz_inner (mode={}, add_indexes={add_indexes}) seed: {seed}",
            mode.as_str()
        );
        ...
        for iter in 0..iterations {
            if iter % (iterations / 100).max(1) == 0 {
                println!(
                    "join_fuzz_inner(mode={}, add_indexes={}) iter {}/{}",
                    mode.as_str(),
                    add_indexes,
                    iter + 1,
                    iterations
            ...
            let limbo_rows = match limbo_exec_rows_fallible(&db, &limbo_conn, &query) {
                Ok(rows) => rows,
                Err(err) => {
                    panic!(
                        "JOIN FUZZ LIMBO ERROR (mode={}, add_indexes={})\nseed: {}\niteration: {}\nquery: {}\nerror: {:?}\nsqlite path: {:?}\nlimbo path: {:?}",
                        mode.as_str(),
                        add_indexes,
                        seed,
                        iter,
            ...
            if sqlite_rows != limbo_rows {
                if std::env::var_os("JOIN_FUZZ_DUMP_REPRO").is_some() {
                    ...
                    eprintln!(
                        "JOIN FUZZ mismatch repro dump disabled. Set JOIN_FUZZ_DUMP_REPRO=1 to print DDL/DML."
                    );
                }

                panic!(
                "JOIN FUZZ MISMATCH (mode={}, add_indexes={})\nseed: {}\niteration: {}\nquery: {}\n\
                 sqlite ({} rows): {:?}\nlimbo ({} rows): {:?}\nsqlite path: {:?}\nlimbo path: {:?}",
                mode.as_str(),
                add_indexes,
                seed,
    ...
    pub fn join_fuzz_unindexed_keys(db: TempDatabase) {
        join_fuzz_inner(db, false, 2000, 200, JoinFuzzMode::Mixed);
    }

    #[turso_macros::test(mvcc)]
    pub fn join_fuzz_indexed_keys(db: TempDatabase) {
        join_fuzz_inner(db, true, 2000, 200, JoinFuzzMode::Mixed);
    }

    #[turso_macros::test(mvcc)]
    pub fn join_fuzz_full_outer_unindexed_keys(db: TempDatabase) {
        join_fuzz_inner(db, false, 1200, 200, JoinFuzzMode::FullOuterOnly);
    }

    #[turso_macros::test(mvcc)]
    pub fn join_fuzz_full_outer_indexed_keys(db: TempDatabase) {
        join_fuzz_inner(db, true, 1200, 200, JoinFuzzMode::FullOuterOnly);
    }
...
>>>>>>> 6c9455fcf (Add additional fuzzing for full outer joins)
    // TODO: mvcc indexes
    #[turso_macros::test(mvcc)]
