...
use super::{make_record, OpHashBuildState, Program, ProgramState, Register};
            ...
            OpColumnState::GetColumn => {
                ...
                match cursor_type {
                    ...
                    | CursorType::MaterializedView(_, _) => {
                        ...
                        // Check if this is column 0 (potential rowid alias)
                        // If so, get the rowid instead of using the default
                        if *column == 0 {
                            let cursor_ref = must_be_btree_cursor!(
                                *cursor_id,
                                program.cursor_ref,
                            ...
                            if let Some(rowid_val) = rowid {
                                state.registers[*dest] = Register::Value(Value::Integer(rowid_val));
                                break;
                            }
                        }
